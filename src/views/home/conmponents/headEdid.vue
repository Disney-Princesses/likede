<template>
  <!-- 头部 -->
  <el-row class="el-row">
    <el-col :span="10">
      <div class="head">
        <div class="title">
          工单统计<span>{{ startDate }}~{{ endDate }}</span>
        </div>
        <div class="body">
          <div class="child">
            <div class="text1">{{ ticketsList.total }}</div>
            <div class="text2">工单总数（个）</div>
          </div>
          <div class="child">
            <div class="text1">{{ ticketsList.completedTotal }}</div>
            <div class="text2">完成工单（个）</div>
          </div>
          <div class="child">
            <div class="text1">{{ ticketsList.progressTotal || 0 }}</div>
            <div class="text2">进行工单（个）</div>
          </div>
          <div class="child">
            <div class="text1">{{ ticketsList.cancelTotal }}</div>
            <div class="text2">取消工单（个）</div>
          </div>
        </div>
      </div>
    </el-col>
    <el-col :span="8">
      <div class="head h-right">
        <div class="title">
          销售统计<span>{{ startDate }}~{{ endDate }}</span>
        </div>
        <div class="body market">
          <div class="child">
            <div class="text1">{{ order }}</div>
            <div class="text2">订单量（个）</div>
          </div>
          <div class="child">
            <div class="text1">{{ amount }}</div>
            <div class="text2">销售额（万元）</div>
          </div>
        </div>
      </div>
    </el-col>
  </el-row>
</template>

<script>
import dayjs from 'dayjs'
import {
  getTaskorderCount,
  getTaskReportInfo,
  getTaskoRevenueAmount,
} from '@/api'
export default {
  data() {
    return {
      // 工单数据
      ticketsList: [],
      // 本月开始时间
      start: '',
      // 结束时间
      end: '',
      // 订单数据
      order: '',
      // 销售额
      amount: '',
    }
  },

  created() {
    this.getTaskReportInfo()
    this.getTaskorderCount()
    this.getTaskoRevenueAmount()
  },

  methods: {
    //获取工单数据
    async getTaskReportInfo() {
      this.start = dayjs().startOf('M').format('YYYY-MM-DD HH:mm:ss')
      this.end = dayjs().endOf('D').format('YYYY-MM-DD HH:mm:ss')
      const res = await getTaskReportInfo(this.start, this.end)
      this.ticketsList = res.data[0]
    },
    // 获取订单数据
    async getTaskorderCount() {
      const res = await getTaskorderCount({
        start: this.start,
        end: this.end,
      })
      this.order = res.data
    },
    // 获取销售额
    async getTaskoRevenueAmount() {
      const res = await getTaskoRevenueAmount({
        start: this.start,
        end: this.end,
      })
      res.data = res.data / 10000
      this.amount = res.data
    },
  },
  computed: {
    startDate() {
      return dayjs().startOf('M').format('YYYY.MM.DD')
    },
    endDate() {
      return dayjs().startOf('D').format('YYYY.MM.DD')
    },
  },
}
</script>

<style scoped lang="less">
.el-row {
  box-sizing: border-box;
  margin-left: -10px;
  margin-right: -10px;
  line-height: 1.15;
  .head {
    margin-left: 10px;
    box-sizing: border-box;
    background-color: #e9f3ff;
    background-image: url(data:image/png;base64,),
      url(http://likede2-admin.itheima.net/img/task.66b715b7.png);
    background-repeat: no-repeat, no-repeat;
    background-position: 0 0, calc(100% - 12px) 100%;
    display: flex;
    flex-direction: column;
    height: calc(25vh - 30px);
    min-height: 166px;
    padding: 20px;
    border-radius: 20px;
    margin-right: 20px;
    .title {
      font-size: 16px;
      font-weight: 600;
      color: #333;
      span {
        margin-left: 10px;
        font-size: 12px;
        font-family: PingFangSC-Regular, PingFang SC;
        font-weight: 400;
        color: #999;
      }
    }
    .body {
      flex: 1;
      display: -webkit-box;
      display: -ms-flexbox;
      display: flex;
      .child {
        display: flex;
        align-items: left;
        justify-content: center;
        text-align: center;
        flex: 1;
        flex-direction: column;
        .text1 {
          text-shadow: 2px 4px 7px rgb(85 132 255 / 50%);
          color: #072074;
          height: 50px;
          font-size: 36px;
          font-weight: 600;
          line-height: 50px;
          text-shadow: 2px 4px 7px rgb(85 132 255 / 50%);
        }
        .text2 {
          color: #91a7dc;
          height: 17px;
          margin-top: 3px;
          font-size: 12px;
          font-weight: 400;
          color: #91a7dc;
          line-height: 17px;
        }
      }
    }
    .market {
      color: pink;
      .child {
        .text1 {
          text-shadow: 2px 4px 7px rgb(255 99 85 / 50%);
          color: #ff5757;
        }
        .text2 {
          color: #de9690;
        }
      }
    }
  }
  .h-right {
    background: #fbefe8
      url(http://likede2-admin.itheima.net/img/sale.606b0d8c.png) no-repeat
      calc(100% - 12px) 100%;
  }
}
</style>
